...
export default function MovieCard(){
	...
	const firstHalfElementRef=useCallback(node1=>{
		...
		observer1.current= new IntersectionObserver(entries=>{
			if(entries[0].isIntersecting && !memoNode.includes(node1) && curr<=totalPage && entries[0].intersectionRatio>0){
				memoNode.push(node1);
	...
	const lastHalfElementRef=useCallback(node2=>{
		...
		observer2.current= new IntersectionObserver(entries=>{
			if(entries[0].isIntersecting && !memoNode.includes(node2) && curr<=totalPage && entries[0].intersectionRatio>0){
				setCurr(curr+1);
				memoNode.push(node2);
			}
		})
	...
	return(
		...
		stack.movieStack.map((mov,index)=>{
			if((index+1)%5==0 && (index+1)%10!=0){
				return (
					<div>
						<Card ref={firstHalfElementRef} className={classes.root} key={index}>
				...
				return(
					<div>
						<Card ref={lastHalfElementRef} className={classes.root} key={index}>
								<CardActionArea onClick={()=>{
										let image;
										if(mov.Poster =='N/A'){
											image='https://image.shutterstock.com/image-vector/image-not-available-icon-260nw-1036295239.jpg';
